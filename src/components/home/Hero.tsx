...
export default function Hero({ featuredFilms }: HeroProps) {
  ...
  return (
    <section className="relative min-h-[100dvh] w-full flex items-center md:items-center overflow-hidden bg-[#050505]">
      {/* Background Slides */}
      <div className="absolute inset-0 z-0">
        {featuredFilms.map((film, index) => (
          <div
      ...
      {/* Main Content Container - pt-32 added for mobile transparency padding */}
      <div className="relative z-10 w-full max-w-[1800px] mx-auto px-6 md:px-12 pt-32 pb-12 md:pt-0">
        <div className="flex flex-col md:flex-row items-center gap-12 md:gap-24">
          
          {/* Poster Slot (Hidden on very small screens to save space) */}
          <div className="hidden lg:block w-1/3 aspect-[3/4] max-w-[420px] relative group overflow-hidden">
            <div
              className={`w-full h-full transition-all duration-700 transform ${
                isTransitioning ? 'scale-95 opacity-0 -translate-x-10' : 'scale-100 opacity-100 translate-x-0'
              }`}
          ...
          <div className="flex-1 space-y-8 md:space-y-10">
            <div
              className={`space-y-4 md:space-y-6 transition-all duration-700 delay-100 ${
                isTransitioning ? 'opacity-0 translate-y-8' : 'opacity-100 translate-y-0'
              }`}
            >
              <div className="flex items-center gap-4 md:gap-6">
                <span className="w-8 md:w-12 h-[1px] bg-white/20" />
                <span className="text-[8px] md:text-[10px] font-bold uppercase tracking-[0.6em] text-white/40">
                  Premiere Selection • Vol. {currentIndex + 1}
                </span>
              </div>
              <h1 className="text-5xl sm:text-6xl md:text-[8vw] font-black italic uppercase tracking-tighter leading-[0.85] mix-blend-difference break-words">
                {currentFilm.title.split(' ')[0]} <br />
                <span className="text-transparent stroke-white stroke-1">
                  {currentFilm.title.split(' ').slice(1).join(' ')}
                </span>
            ...
            <div
              className={`flex flex-col md:flex-row gap-8 md:gap-12 items-start md:items-center transition-all duration-700 delay-200 ${
                isTransitioning ? 'opacity-0 translate-y-8' : 'opacity-100 translate-y-0'
              }`}
            >
              <p className="text-white/40 text-base md:text-xl max-w-md font-light italic leading-relaxed">
                {currentFilm.description}
              </p>
              
              <div className="flex gap-8 border-l border-white/10 pl-6 md:pl-8">
                <div className="flex flex-col">
                  <span className="text-[8px] uppercase tracking-widest text-white/30">Category</span>
                  <span className="text-xs md:text-sm font-bold uppercase tracking-widest">
                    {currentFilm.genre ?? currentFilm.genres?.[0]}
                  </span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[8px] uppercase tracking-widest text-white/30">Language</span>
                  <span className="text-xs md:text-sm font-bold uppercase tracking-widest">{currentFilm.language}</span>
                </div>
              </div>
            </div>

            <div
              className={`flex items-center gap-8 md:gap-12 pt-4 transition-all duration-700 delay-300 ${
                isTransitioning ? 'opacity-0 translate-y-8' : 'opacity-100 translate-y-0'
              }`}
            >
              <button
                onClick={handleWatch}
                className="group flex items-center gap-4 md:gap-6 text-[10px] md:text-[11px] font-black uppercase tracking-[0.3em] transition-all"
              >
                <div className="w-14 h-14 md:w-20 md:h-20 rounded-full border border-white/20 flex items-center justify-center group-hover:bg-white group-hover:border-white transition-all duration-500">
                  <Play size={18} className="text-white group-hover:text-black fill-current ml-1 md:size-[24px]" />
                </div>
                <span className="border-b border-transparent group-hover:border-white pb-1">Begin Experience</span>
              </button>
            </div>
          </div>
        </div>

        {/* Carousel Navigation - Optimized for Phone */}
        <div className="mt-12 md:mt-20 flex flex-col md:flex-row justify-between items-center md:items-end gap-8">
          <div className="flex items-center gap-8 md:gap-12 w-full justify-between md:justify-start">
            ...
            <div className="flex gap-1 items-end">
              <span className="text-2xl md:text-3xl font-black italic leading-none">
                {String(currentIndex + 1).padStart(2, '0')}
              </span>
              <span className="text-white/20 font-bold mb-1 text-xs md:text-base">
                / {String(featuredFilms.length).padStart(2, '0')}
              </span>
            </div>
          </div>

          {/* Progress Bars - Hidden on mobile to avoid clutter, or made smaller */}
          <div className="hidden sm:flex gap-4 w-full md:w-[600px]">
            {featuredFilms.map((_, idx) => (
              <div
                key={idx}
                onClick={() => setCurrentIndex(idx)}
